...
public class DefaultStudentService implements StudentService {
    ...
    public List<Student> findStudentsWithMathRatingMoreThanAvgAndTakeEngExam() {
        ...
        double avg = average.orElseThrow(NoSuchElementException::new);
    ...
    public Student findStudentWithMaxEngRating() {
        return studentRepository.findAll().stream()
                ...
                .max((student, t1) -> {
                    Optional<Exam> score1 = student.getExams().stream().filter(exam -> exam.getType() == Exam.Type.ENGLISH).findAny();
                    Optional<Exam> score2 = t1.getExams().stream().filter(exam -> exam.getType() == Exam.Type.ENGLISH).findAny();
